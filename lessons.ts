...
function buildSignalSnapshot(perf: PerformanceRecord): Record<string, unknown> | null {
  const snapshot: Record<string, unknown> = { ...(perf.signal_snapshot || {}) };
...
export async function recordPerformance(perf: PerformanceRecord): Promise<void> {
  ...
  const pnl_pct = (perf.initial_value_usd ?? 0) > 0
...
function derivLesson(perf: PerformanceRecord): Lesson | null {
  const tags: string[] = [];
  const feeYieldPct = (perf.initial_value_usd ?? 0) > 0
    ? (((perf.fees_earned_usd || 0)) / perf.initial_value_usd!) * 100
    : 0;

  // Categorize outcome
  const outcome = (perf.pnl_pct ?? 0) >= 5 ? "good"
    : ((perf.pnl_pct ?? 0) >= 0 && feeYieldPct >= 2) ? "good"
    : (perf.pnl_pct ?? 0) >= 0 ? "neutral"
    : (perf.pnl_pct ?? 0) >= -5 ? "poor"
    : "bad";
  ...
  if (outcome === "good" || outcome === "bad") {
    if ((perf.range_efficiency ?? 0) < 30 && outcome === "bad") {
      rule = `AVOID: ${perf.pool_name}-type pools (volatility=${perf.volatility}, bin_step=${perf.bin_step}) with strategy="${perf.strategy}" — went OOR ${100 - (perf.range_efficiency ?? 0)}% of the time. Consider wider bin_range or bid_ask strategy.`;
      tags.push("oor", perf.strategy ?? "", `volatility_${Math.round(perf.volatility ?? 0)}`);
    } else if ((perf.range_efficiency ?? 0) > 80 && outcome === "good") {
...
export function evolveThresholds(
  ...
  cfg: Config
): { changes: Record<string, number>; rationale: Record<string, string> } | null {
  if (!perfData || perfData.length < MIN_EVOLVE_POSITIONS) return null;

  const winners = perfData.filter((p) => (p.pnl_pct ?? 0) > 0);
  const losers  = perfData.filter((p) => (p.pnl_pct ?? 0) < -5);
...
export function listLessons({
  ...
  total: number;
  lessons: Array<{ id: number; rule: string; tags: string[]; outcome: string; pinned: boolean; role: string; created_at?: string }>;
} {
  const data = load();
...
export function getLessonsForPrompt(
  opts: { agentType?: string; maxLessons?: number } | number = {}
): string | null {
...
export async function getPerformanceHistory({
  ...
  if (volatility_min != null) filtered = filtered.filter((p) => Number.isFinite(p.volatility) && (p.volatility as number) >= volatility_min);
  if (close_reason) filtered = filtered.filter((p) => String(p.close_reason || "").toLowerCase().includes(close_reason.toLowerCase()));
  if (min_pnl_pct != null) filtered = filtered.filter((p) => Number.isFinite(p.pnl_pct) && (p.pnl_pct as number) >= min_pnl_pct);
  if (max_pnl_pct != null) filtered = filtered.filter((p) => Number.isFinite(p.pnl_pct) && (p.pnl_pct as number) <= max_pnl_pct);

  // Sort by closed_at desc, limit
  filtered.sort((a, b) => String(b.closed_at || "").localeCompare(String(a.closed_at || "")));
  filtered = filtered.slice(0, limit);

  const totalPnl = filtered.reduce((s, r) => s + ((r.pnl_usd as number) ?? 0), 0);
  const wins = filtered.filter((r) => ((r.pnl_usd as number) ?? 0) > 0).length;
