...
const DEFAULT_WEIGHTS: Record<SignalName, number> = Object.fromEntries(
 ...
 * @returns Object with changes array and weights map
 */
export function recalculateWeights(
  perfData: PerformanceRecord[],
  cfg: Config
): { changes: Array<{ signal: string; from: number; to: number; lift: number; action: string }>; weights: Record<string, number> } {
  const { darwin } = cfg;
  ...
  const recent = perfData.filter((p) => {
    const ts = (p.recorded_at as string) || (p.closed_at as string) || (p.deployed_at as string);
  ...
  const wins   = recent.filter((p) => ((p.pnl_usd as number) ?? 0) > 0);
  const losses = recent.filter((p) => ((p.pnl_usd as number) ?? 0) <= 0);
...
function computeLift(signal: SignalName, wins: PerfEntry[], losses: PerfEntry[], minSamples: number): number | null {
...
function computeNumericLift(signal: SignalName, wins: PerfEntry[], losses: PerfEntry[], minSamples: number): number | null {
...
function computeBooleanLift(signal: SignalName, wins: PerfEntry[], losses: PerfEntry[], minSamples: number): number | null {
...
function computeCategoricalLift(signal: SignalName, wins: PerfEntry[], losses: PerfEntry[], minSamples: number): number | null {
  const allEntries = [...wins.map((w) => ({ w: true, snap: w })), ...losses.map((l) => ({ w: false, snap: l }))];
...
function extractNumeric(signal: SignalName, entries: PerfEntry[]): number[] {
  const vals: number[] = [];
...
function getEntrySignalSnapshot(entry: PerfEntry): Record<string, unknown> | null {
  if (entry.signal_snapshot) return entry.signal_snapshot as Record<string, unknown>;
