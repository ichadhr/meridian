...
function save(data: StrategyDB): void {
  saveJsonRecord(STRATEGY_FILE, data);
}

// ─── Default Strategies ─────────────────────────────────────────
const DEFAULT_STRATEGIES: Record<string, Omit<StrategyEntry, "added_at" | "updated_at">> = {
  custom_ratio_spot: {
 ...
 */
export function getStrategy({ id }: { id?: string }): (StrategyEntry & { is_active: boolean }) | { error: string; available?: string[] } {
 ...
 */
export function setActiveStrategy({ id }: { id?: string }): { error?: string; available?: string[]; active?: string; name?: string } {
  if (!id) return { error: "id required" };
  const db = load();
 ...
 */
export function getActiveStrategy(): StrategyEntry | null {
  const db = load();
