...
function isAdjustedWinRateExcludedReason(reason: unknown): boolean {
  const text = String(reason || "").trim().toLowerCase();
...
function isFeeGeneratingDeploy(deploy: PoolDeploy): boolean {
...
function setBaseMintCooldown(db: Record<string, PoolMemoryEntry>, baseMint: string | null, hours: number, reason: string): string | null {
 ...
 * Called automatically from recordPerformance() in lessons.ts.
 */
export function recordPoolDeploy(
  ...
  if (withPnl.length > 0) {
    entry.avg_pnl_pct = Math.round(
      (withPnl.reduce((s, d) => s + (d.pnl_pct ?? 0), 0) / withPnl.length) * 100
    ) / 100;
    entry.win_rate = Math.round(
      (withPnl.filter((d) => (d.pnl_pct ?? 0) >= 0).length / withPnl.length) * 100
    ) / 100;
  }
  ...
  entry.adjusted_win_rate = adjusted.length > 0
    ? Math.round((adjusted.filter((d) => (d.pnl_pct ?? 0) >= 0).length / adjusted.length) * 10000) / 100
    : 0;
  ...
  if (config.management.repeatDeployCooldownEnabled) {
    ...
    const scope: "pool" | "token" | "both" = ["pool", "token", "both"].includes(rawScope) ? (rawScope as "pool" | "token" | "both") : "token";
 ...
 */
export function getPoolMemory({ pool_address }: { pool_address?: string }): Record<string, unknown> {
...
export function recallForPool(poolAddress: string): string | null {
  ...
  if (snaps.length >= 2) {
    ...
    const pnlTrend = last.pnl_pct != null && first.pnl_pct != null
      ...
      : null;
    const oorCount = snaps.filter((s: PoolSnapshot) => s.in_range === false).length;
 ...
 */
export function addPoolNote({ pool_address, note }: { pool_address?: string; note?: string }): Record<string, unknown> {
  if (!pool_address) return { error: "pool_address required" };
