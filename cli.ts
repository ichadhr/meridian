...
function die(msg: string, extra: any = {}): never {
...
switch (subcommand) {
  ...
  case "pnl": {
    ...
    if (tracked?.strategy) (pnl as any).strategy = tracked.strategy;
    if (tracked?.instruction) (pnl as any).instruction = tracked.instruction;
    out(pnl);
    break;
  }
  ...
  case "candidates": {
    ...
    const pools = raw.candidates || (raw as any).pools || [];

    const enriched: any[] = [];
    for (const poolRaw of pools) {
      const pool: any = poolRaw;
      ...
      const ti = tokenInfo.status === "fulfilled" ? (tokenInfo.value as any)?.results?.[0] : null;
      enriched.push({
        pool: pool.pool,
        name: pool.name,
  ...
  case "deploy": {
    ...
    const { executeTool } = await import("./llm/index.js");
  ...
  case "study": {
    ...
    const { studyTopLPers } = await import("./llm/index.js");
  ...
  case "discord-signals": {
    ...
    const pending = signals.filter((s: any) => s.status === "pending");
    const processed = signals.filter((s: any) => s.status !== "pending");
    out({
