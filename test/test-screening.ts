...
async function main(): Promise<void> {
  ...
  const top = await discoverPools({ page_size: 10, timeframe: "24h", category: "top" }) as any;
  ...
  const trending = await discoverPools({ page_size: 5, timeframe: "1h", category: "trending" }) as any;
