// PnL math helper functions extracted from dlmm.ts.
// Pure helpers (no Meteora SDK / RPC / wallet deps) plus the Agent Meridian
// raw open-positions fetcher, since it's only used by the PnL flow.
//
// The functions in this file were previously local helpers in dlmm.ts; moving
// them here keeps dlmm.ts focused on deploy/close orchestration and lets unit
// tests import the math helpers without pulling in envcrypt, RPC, or wallet
// modules.

import { config } from "../../config/index.js";
import { getAndClearStagedSignals } from "../../core/index.js";
import { agentMeridianJson, getAgentMeridianHeaders } from "../hivemind/index.js";
...
export function roundNum(value: unknown, decimals = 4): number {
...
export function resolvePerformanceSignalSnapshot({ poolAddress, baseMint, tracked }: { poolAddress: string; baseMint: string; tracked: any }): Record<string, unknown> | null {
...
export async function fetchRawOpenPositionsFromMeridian({ walletAddress, agentId }: { walletAddress: string; agentId?: string }): Promise<any> {
