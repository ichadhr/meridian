...
const PUBLIC_HEADERS: Record<string, string> = { "Ok-Access-Client-type": "agent-cli" };
...
const serverEnrichmentCache = new Map<string, { at: number; promise: Promise<any> }>();
...
function agentMeridianHeaders(): Record<string, string> {
...
async function fetchServerOkxEnrichment(
  tokenAddress: string,
  ...
  const promise = fetch(url, { headers: agentMeridianHeaders() })
    .then(async (res) => {
      ...
      if (!res.ok) {
        throw new Error(
          payload?.error || `Agent Meridian OKX enrichment ${res.status}`,
        );
      }
      return payload;
    })
...
async function getServerOkxEnrichmentOrNull(
  tokenAddress: string,
  chainIndex = CHAIN_SOLANA,
): Promise<any> {
...
function collectRiskEntries(section: Record<string, unknown> | undefined): RiskEntry[] {
...
export async function getRiskFlags(
  ...
  const hasRisk = (riskKey: string): boolean =>
    entries.some(
      (entry) => entry?.riskKey === riskKey && isAffirmative(entry?.newRiskLabel),
    );
...
export async function getClusterList(
  ...
  if (Array.isArray(serverPayload?.clusters))
    return serverPayload.clusters.slice(0, limit);
  ...
  return raw.slice(0, limit).map((c: any) => {
    const hasKol = (c.clusterAddressList || []).some((a: any) => a.isKol);
    return {
