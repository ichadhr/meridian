...
interface PoolDiscoveryRecord {
  ...
  dlmm_params?: { bin_step?: number; [key: string]: unknown };
  pool_config?: { bin_step?: number; [key: string]: unknown };
  pool_type?: string;
  fee_pct?: number;
  tvl?: number;
  active_tvl?: number;
  ...
  base_mint?: string;
  base?: { mint?: string; symbol?: string; [key: string]: unknown };
  ...
  discord_signal_last_seen_at?: string;
  [key: string]: unknown;
}
...
interface CondensedPool {
  ...
  active_positions: number | undefined;
  active_pct: number | null;
  open_positions: number | undefined;
  discord_signal: boolean;
  discord_signal_count: number;
  discord_signal_seen_count: number;
  discord_signal_last_seen_at: string | null | undefined;
  ...
  swap_count: number | undefined;
  unique_traders: number | undefined;
  // Enriched fields (added after condensation)
  is_pvp?: boolean;
  pvp_risk?: string;
  pvp_symbol?: string;
  ...
  smart_money_buy?: unknown;
  dev_sold_all?: unknown;
  dex_boost?: unknown;
  dex_screener_paid?: unknown;
  is_rugpull?: boolean;
  is_wash?: boolean;
  ...
  indicator_confirmation?: unknown;
  [key: string]: unknown;
}

interface FilteredExample {
  name: string;
  reason: string;
}

interface DiscordSignalCandidate {
  discovery_pool?: PoolDiscoveryRecord;
  source_count?: number;
  seen_count?: number;
...
interface DiscoverPoolsResult {
  total: number | undefined;
  pools: CondensedPool[];
  filtered_examples: FilteredExample[];
}

interface GetTopCandidatesResult {
...
function scoreCandidate(pool: PoolDiscoveryRecord): number {
...
function includesCaseInsensitive(values: unknown, value: unknown): boolean {
  if (!Array.isArray(values) || values.length === 0 || !value) return false;
  const needle = String(value).toLowerCase();
  return values.some((entry: unknown) => String(entry).toLowerCase() === needle);
}

function getPoolLaunchpad(pool: PoolDiscoveryRecord): string | null {
...
function getPoolBaseMint(pool: PoolDiscoveryRecord): string | null {
...
function getRawPoolScreeningRejectReason(pool: PoolDiscoveryRecord, s: ScreeningConfig): string | null {
  const base = pool?.token_x || {};
...
async function fetchDiscordSignalCandidates(): Promise<DiscordSignalCandidate[]> {
...
async function applyVolatilityTimeframe(rawPools: PoolDiscoveryRecord[], sourceTimeframe: string): Promise<PoolDiscoveryRecord[]> {
  ...
  const metricsByPool = new Map<string, PoolMetrics>();
...
async function enrichDiscordSignalLaunchpads(rawPools: PoolDiscoveryRecord[]): Promise<void> {
...
async function findRivalPool(mint: string): Promise<RivalPool | null> {
  ...
  const data = await res.json();
  const pools: RivalPool[] = Array.isArray(data?.data) ? data.data : [];
...
async function enrichPvpRisk(pools: CondensedPool[]): Promise<void> {
  const shortlist = [...pools]
    .sort((a, b) => scoreCandidate(b as unknown as PoolDiscoveryRecord) - scoreCandidate(a as unknown as PoolDiscoveryRecord))
  ...
  await Promise.all(shortlist.map(async (pool) => {
    ...
    for (const rival of rivalAssets) {
      const rivalHolders = Number((rival as any)?.holderCount || 0);
      const rivalFees = Number((rival as any)?.fees || 0);
      if (rivalHolders < PVP_MIN_HOLDERS || rivalFees < PVP_MIN_GLOBAL_FEES_SOL) continue;

      const rivalPool = await findRivalPool((rival as any).id).catch(() => null);
      if (!rivalPool) continue;

      pool.is_pvp = true;
      pool.pvp_risk = "high";
      pool.pvp_symbol = pool.base?.symbol || symbol;
      pool.pvp_rival_name = (rival as any)?.name || pool.pvp_symbol;
      pool.pvp_rival_mint = (rival as any).id;
      pool.pvp_rival_pool = rivalPool.address;
      pool.pvp_rival_tvl = round(Number(rivalPool.tvl || 0)) ?? 0;
      pool.pvp_rival_holders = rivalHolders;
      pool.pvp_rival_fees = Number(rivalFees.toFixed(2));
      log("screening", `PVP guard: ${pool.name} has active rival ${pool.pvp_rival_name} (${(rival as any).id.slice(0, 8)})`);
      break;
    }
  }));
}
 ...
 */
async function refreshDiscordOnlyPools(pools: PoolDiscoveryRecord[], timeframe: string): Promise<void> {
...
export async function discoverPools({
  ...
  let rawPools: PoolDiscoveryRecord[] = Array.isArray(data.data) ? data.data : [];

  if (config.screening.useDiscordSignals) {
    const signalCandidates = await fetchDiscordSignalCandidates().catch((error: Error) => {
    ...
    const signalPools = signalCandidates
      .map((candidate) => {
        ...
        } as PoolDiscoveryRecord;
      })
      .filter(Boolean) as PoolDiscoveryRecord[];
    ...
    if (config.screening.discordSignalMode === "only") {
      ...
      const discordOnlyPools: PoolDiscoveryRecord[] = [];
  ...
  let pools = condensed.filter((p) => {
    ...
    if (mint && isBlacklisted(mint)) {
      log("blacklist", `Filtered blacklisted token ${p.base?.symbol} (${mint.slice(0, 8)}) in pool ${p.name}`);
      return false;
    }
    if (p.dev && isDevBlocked(p.dev as string)) {
      log("dev_blocklist", `Filtered blocked deployer ${(p.dev as string)?.slice(0, 8)} token ${p.base?.symbol} in pool ${p.name}`);
      return false;
    }
    return true;
  });
  ...
  if (Object.keys(blockedDevs).length > 0) {
    ...
    if (missingDev.length > 0) {
      ...
      for (const r of devResults) {
        if (r.status === "fulfilled" && r.value.pool) devMap[r.value.pool] = r.value.dev;
      }
      pools = pools.filter((p) => {
        const dev = devMap[p.pool!];
        if (dev) p.dev = dev; // enrich in-place
        if (dev && isDevBlocked(dev as string)) {
          log("dev_blocklist", `Filtered blocked deployer (jup) ${(dev as string).slice(0, 8)} token ${p.base?.symbol}`);
          return false;
        }
        return true;
      });
...
export async function getTopCandidates({ limit = 10 }: {
  ...
  const eligible = pools
    .filter((p) => {
      ...
      const mint = p.base?.mint;
      if (mint && occupiedMints.has(mint)) {
      ...
      if (mint && isBaseMintOnCooldown(mint)) {
  ...
  if (eligible.length > 0) {
    ...
    if (athFilter != null) {
      const threshold = 100 + (athFilter as number); // e.g. -20 → threshold = 80 (price must be <= 80% of ATH)
      const before = eligible.length;
      eligible.splice(0, eligible.length, ...eligible.filter((p) => {
    ...
    const before = eligible.length;
    const filteredDev = eligible.filter((p) => {
      if (p.dev && isDevBlocked(p.dev as string)) {
        log("dev_blocklist", `Filtered blocked deployer (okx) ${(p.dev as string).slice(0, 8)} token ${p.base?.symbol}`);
 ...
 */
function condensePool(p: PoolDiscoveryRecord): CondensedPool {
  return {
    pool: p.pool_address,
    name: p.name,
    base: {
...
function round(n: unknown): number | null {
  return n != null ? Math.round(n as number) : null;
}

function fix(n: unknown, decimals: number): number | null {
  const value = Number(n);
  return Number.isFinite(value) ? Number(value.toFixed(decimals)) : null;
}

function pushFilteredReason(list: FilteredExample[], pool: CondensedPool, reason: string): void {
  if (!list || !pool) return;
  list.push({
