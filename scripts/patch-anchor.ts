...
if (fs.existsSync(dlmmMjs)) {
  ...
  function removeBNFromSpecifiers(specifiers: string): string {
    return specifiers
      .split(",")
      .map(s => s.trim())
  ...
  src = src.replace(
    ...
    (_, before: string, alias: string, after: string) => {
  ...
  src = src.replace(
    ...
    (_: string, before: string, after: string) => {
      const remaining = removeBNFromSpecifiers(before + "," + after);
